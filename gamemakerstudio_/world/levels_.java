 ...
 * @author ACER
 */
public class levels_ implements MouseMotionListener, MouseListener, KeyListener {
    ...
    public void mouseReleased(MouseEvent e) {
        if (lazyDelayFix == 0) {
            if (game.gameState == game_.STATE.LevelSelect) {
                ...
                if (page > 1) if (mouseOver(mx, my, 16, 366, 20, 20)) {
                    page--;
...
//                if (game_.music) audioplayer_.getSound("click_sound").play();
            ...
            if (levelid == 1) {
                ...
                if (scoreKeep >= tpm) {
                    ...
                    if (handler.total_beats == 69 /*nice*/) {
                        handler.addObject(new ghost_(r.nextInt(game_.WIDTH - 1), r.nextInt(game_.HEIGHT - 1), ID.GHOST, 1, 1,0));
    ...
    public void render(Graphics g) {
        if (game.gameState == game_.STATE.LevelSelect) {
            ...
            g.drawString("Page: " + page, game.WIDTH -64, 38);
            // prev and next
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (game.gameState == game_.STATE.LevelSelect) {
            if (!game.isSelecting) {
                ...
                handler.removeAllSelectedObjectsExceptPlayers(ID.CURSORSELECT);
                handler.addObject(new CURSOR_SELECT(gridSelectX + 10, gridSelectY + 10, ID.CURSORSELECT));
                // run level
                if (key == KeyEvent.VK_ESCAPE || key == KeyEvent.VK_ENTER) {
                    if (key == KeyEvent.VK_ENTER) {
                        ...
                        if (gridSelectX == 0 && gridSelectY == 350 && page > 1){
                            page--;
                        } else if (gridSelectX == 600 && gridSelectY == 350){
                            page++;
                        } else {
                            for (int x = 60; x <= 560; x += 100) {
                                for (int y = 60; y <= 660; y += 100) {
