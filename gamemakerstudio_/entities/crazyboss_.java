...
public class crazyboss_ extends gameobject_ {
    ...
    public crazyboss_(float x, float y, ID id, handler_ handler, float velXCustom, float velYCustom) {
        super(x, y, id);
    ...
    public void tick() {
        ...
        if (customRotateByTick) {
            ...
            if (!increment) rotateTick -= rotateThisTick;
            rotateTick = game_.clamp(rotateTick, minRotate, maxRotate);
        } else rotateTick = 0;
        handler.addObject(new tnt_ ((int) x + 128, (int) y + 128, ID.BasicEnemy, handler));
    }
