...
public class bullethellgenerator_ extends gameobject_ implements KeyListener, MouseListener, ActionListener {
    ...
    public void tick() {
        ...
        if (spawnTimer == 0) {
            ...
            for (int i = 0; i <= numberOfProjectiles; i++) {
                double projectileDirXPosition = ((startPoint.getX() + 10) + Math.sin((angle * Math.PI) / 180) * radius);
                double projectileDirYPosition = ((startPoint.getY() + 10) + Math.cos((angle * Math.PI) / 180) * radius);
            ...
            if (maxSpinRate != 0)
                spinRate = game.clampDouble(spinRate, -maxSpinRate, maxSpinRate);
    ...
    // the impossible math, do not use it at all cost
