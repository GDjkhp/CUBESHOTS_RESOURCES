...
public class portalred_ extends gameobject_ {
    handler_ handler;
    int frames = 0;
    ArrayList<gameobject_> holdEntity = new ArrayList<gameobject_>();
    public portalred_(float x, float y, ID id, handler_ handler, int velX, int velY) {
        super(x, y, id);
        width = 64;
        height = 64;
        this.velX = velX;
            ...
            if (ignoreList(tempObject)) {
                if (tempObject.getBounds() != null){
                    if (getBounds().contains(tempObject.getBounds()) && !tempObject.isTeleporting()){
                        ...
                        gameobject_ tp = handler.getObject(ID.PortalBlue);
...
//                System.out.println("red: the playa escaped (" + holdEntity.get(i).getId() + ")");
    ...
    public void render(Graphics g) {
        ...
        if (frames == 90)
            frames = 0;
        if (frames < 15)
            g.drawImage(assets_.maxRed1, (int)x - 32, (int)y - 32, null);
        if (frames >= 15 && frames < 30)
            g.drawImage(assets_.maxRed2, (int)x - 32, (int)y - 32, null);
        if (frames >= 30 && frames < 45)
            g.drawImage(assets_.maxRed3, (int)x - 32, (int)y - 32, null);
        if (frames >= 45 && frames < 60)
            g.drawImage(assets_.maxRed4, (int)x - 32, (int)y - 32, null);
